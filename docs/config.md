...
### Изменение метаданных объектов слоев (ключ gridAnalysis)
[nsGmx.GridAnalysis](https://github.com/ScanEx/GridAnalysis)
- `<String> apiKey` - ключ доступа
- `<String> serviceBase` - адрес сервисов Геомиксера
- `<String>[] layers` - список идентификаторов слоев
- `<String> column` - редактируемый атрибут объекта
- `menus` - элементы меню в виде [{'ключ': 'значение'}, ...]
